...
router.get('/', async (req:any, res:any, next:any) => {
  ...
  const investmentsHoldings = new Array();
  const investmentsSecurities = new Array();
  ...
  for (let i = 0; i < access_tokens.length; i++) {
    ...
    try {
        ...
        investmentsHoldings.push(response.data.holdings);
        investmentsSecurities.push(response.data.securities);
  ...
  const finalResponse = {
    accounts: accounts,
    investmentsHoldings: investmentsHoldings,
    investmentsSecurities: investmentsSecurities,
    statusCode: 200,
